...
def run_graph_test():
    ...
    try:
        ...
        table = Table(
            title=f"InBody Analysis for {result_profile.name}",
            show_header=True,
        ...
        table.add_row(
            "Skeletal Muscle Mass (SMM)", str(scan.skeletal_muscle_mass_kg), "kg"
        )
        table.add_row("Body Fat (PBF)", str(scan.body_fat_percent), "%")
        table.add_row(
            "Basal Metabolic Rate (BMR)", str(scan.basal_metabolic_rate), "kcal"
        )
        table.add_row(
            "Visceral Fat Level", str(scan.visceral_fat_level or "N/A"), "Level"
        )
        ...
        if scan.segmental_muscle:
            ...
            seg_table = Table(
                title="💪 Segmental Lean Analysis (Muscle Mass)", box=None
            )
