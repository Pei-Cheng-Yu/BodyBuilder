...
class BasicScanMetrics(BaseModel):
    ...
    visceral_fat_level: Optional[int] = Field(None, description="Visceral Fat Level 內臟脂肪等級 (Level 1-20)")
    inbody_score: Optional[int] = Field(None, description="InBody Score 總分")
    curve_type: str = Field(..., description="The shape formed by Weight/Muscle/Fat bars. Options: 'C-Shape', 'I-Shape', 'D-Shape'")
...
class InbodyMetrics(BaseModel):
    ...
    def tdee(self) -> float:
        ...
        return round(self.basal_metabolic_rate * multipliers.get(self.activity_level, 1.2), 0)
class UserProfile(BaseModel):
    name: Optional[str]
    gender: Literal["Male", "Female"]
    age: int = Field(..., gt=0, lt=120)
    
    injuries: list[str] = Field(default_factory=list, description="傷病史，如 ['knee_pain', 'lower_back']")
    activity_level: Literal["Sedentary", "Lightly Active", "Moderately Active", "Very Active"] = "Sedentary"
